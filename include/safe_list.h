...
class SafeList {
	...
	public:
		...
		//int Update(const T& data, bool all, std::function<void(T&)> p);
		template<typename Predicate>
		...
		//void Remove(std::function<void(T&)> p);

		template<typename Predicate>
		void RemoveAll(Predicate p);
		//void RemoveAll(std::function<void(T&)> p);

		template<typename Predicate>
		int UpdateAll(Predicate p);
		//int UpdateAll(std::function<void(T&)> p);

	public:
		template<typename Predicate>
		void Run(Predicate p);
		// void Run(std::function<void(T&)> p);
	private:
		node* m_head;
		size_t m_len;
};
...
template<typename T>
void SafeList<T>::ForEach(std::function<void(T&)> f) {
		node* curr = m_head;
	std::unique_lock<std::mutex> lk(m_head->m_mutex);
	node* next = nullptr;
	while (next = curr->next) {
